...
维护唯一事实源和学习闭环。`NovelOS/04-canon/entity-state-ledger.json` 是机器佳源，`entity-state-view.md` 是确定性人读视图。先生成状态差量提案，再检查批准状态，最后提交 JSON 并运行渲染脚本；不能直接编辑生成视图留下双源分叉。
...
- 提交前由工具管理员运行 `novelos_state_audit`。HARD_FAIL 不提交；REVIEW 只要求核对命中项，不自动恢复伤势、强行推进伏笔或改变人物位置。
